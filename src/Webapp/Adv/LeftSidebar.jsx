...
function NavItem({ icon, label, active = false, onClick }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-300 ease-in-out hover:bg-[#b8d7f4] hover:translate-x-1 ${active ? "bg-[#b8d7f4] text-[#004B80] font-semibold" : "text-[#004B80]"
        }`}
      onClick={onClick}
    >
  ...
  return (
    <div className="w-64 bg-white p-6 flex flex-col h-screen fixed left-0 top-0 shadow-lg border-r border-[#b8d7f4]">
      ...
      <div className="flex items-center gap-2 mb-10 group">
        ...
        <a href="/home">
        <span className="text-xl font-bold text-[#004B80] transition-all duration-300 group-hover:tracking-wide">EERA</span>
        </a>
      </div>
      ...
      {/* Add the keyframes for the fadeIn animation to your global CSS or tailwind config */}
      <style jsx global>{`
        @keyframes fadeIn {
